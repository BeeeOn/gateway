...
class VirtualBelkinWemoSwitch:
	...
	def start(self):
		...
		logging.info("Virtual Belkin WeMo Switch started")
	...
	def __exit__(self, type, value, traceback):
		...
		logging.info("Virtual Belkin WeMo Switch successly stopped.")
	...
	def isUPnPValid(self, msg):
		reSwitchDevice = re.search('ST: urn:Belkin:device:controllee:1', msg)
	...
	def createSetStateMsg(self, body):
		reState = re.search('<BinaryState>([0|1])<\/BinaryState>', body);
		wantedState = reState.group(1)
		...
		result = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">'\
		         ...
		         '<BinaryState>' + wantedState + '</BinaryState>'\
...
if __name__ == "__main__":
	...
	parser.add_argument("--ip", metavar="IP_ADDRESS",
		help="The switch will have this ip address.")
	parser.add_argument("--port", metavar="PORT",
		help="The switch tries to bind on this port number. Has to be put as number.")
	parser.add_argument("--mac", metavar="MAC",
		help="The switch will have this mac address. Has to be put as hex number.")
	parser.add_argument("--serial", metavar="SERIAL_NUMBER",
		help="The switch will have this serial number.")
	...
	with VirtualBelkinWemoSwitch(ip, port, mac, serial) as switch:
		switch.start()
