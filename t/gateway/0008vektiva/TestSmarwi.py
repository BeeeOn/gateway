...
import time
import SensorData
import VektivaModule

class TestSmarwi(VektivaModule.VektivaModule, TCModule.TCModule, NamedPipeModule.NamedPipeModule, GatewayBaseModule.GatewayBaseModule):

	def setUp(self):
		self.vektivaConfig['smarwi'] = True
		super().setUp()
	...
	def test_All(self):
		...
		# init the smarwi
		commander.deviceSetValue(devices[0], "0", "0")
		...
		self.assertEqual(data.data[0][1], 1)
		...
		self.assertEqual(data.deviceID, devices[0])
		self.assertEqual(data.data[0][0], 0)
		self.assertEqual(data.data[0][1], 0)
